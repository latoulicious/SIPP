  ...
  methods: {
    async fetchData() {
      ...
      try {
        ...
        const response = await axios.get("http://localhost:3000/api/remedial");
    ...
    async addNewItem() {
      ...
      try {
        await axios.post("http://localhost:3000/api/remedial", newItem);
        this.items.push({
          ...newItem,
        });
      ...
      if (window.confirm("Are you sure you want to delete this item?")) {
        try {
          const response = await axios.delete(
            `http://localhost:3000/api/remedial/${id}`,
          );

          // Check if deletedRemedial is not null
          if (response.data && response.data.data) {
            const deletedRemedial = response.data.data;

            // Remove the item from the items array
            this.items = this.items.filter(
              (item) => item.id !== deletedRemedial.id,
            );
          }
    ...
    //   try {
    //     const response = await axios.get(`http://localhost:3000/api/remedial/${selectedItemId}`);
    ...
    async openDetailModal(rowIndex) {
      ...
      try {
        const response = await axios.get(
          `http://localhost:3000/api/remedial/${selectedItemId}`,
        );
